...
9. The authenticated dashboard links to `/itinerary`, which renders the client `ItineraryFlow` component. Its upload state previews one JPG/PNG and calls `POST /api/itinerary/upload`; its processing state polls `GET /api/itinerary/job/[id]` every two seconds; its result state renders either the Unsplash-attributed photo variant or the no-photo variant and calls `POST /api/itinerary/[id]/expand`; and its failed state displays the recorded user-safe error with a return-to-upload action.
## 4. The Data Model
